    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                <span class="title"> 浏览记录列表 </span>
            </div>
            <!-- 搜索 -->


            <el-table border :data="list" style="width: 100%" highlight-current-row @selection-change="handleSelectionChange">
                ...
                <el-table-column label="浏览用户" width="80">
                    <template slot-scope="{row}"> {{ row.liulanyonghu }} </template>
                </el-table-column>
        ...
        methods: {
            ...
            batchDelete() {
                this.$confirm("确定删除？").then((res) => {
                    ...
                    this.$post(api.liulanjilu.batch, {
                        // 提交后台
                        delete: 1,
                        ids: ids,
                    })
            ...
            loadList(page) {
                ...
                this.$post(api.liulanjilu.listliulanyonghu, filter)
                    .then((res) => {
                        this.loading = false;
                        if (res.code == api.code.OK) {
            ...
            deleteItem(row) {
                this.$confirm("确定删除数据？", "提示", {
                    ...
                    .then(() => {
                        ...
                        this.loading = true; // 滚动条
                        this.$post(api.liulanjilu.delete, {
                            // 提交后台
                            id: row.id,
                        })
                            .then((res) => {
