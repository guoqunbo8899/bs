        ...
        <el-card class="box-card">
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="参与人数" width="80">
                    <template slot-scope="{row}"> {{ row.canyurenshu }}</template>
                </el-table-column>
                <el-table-column label="活动状态" width="80">
                    <template slot-scope="{row}"> {{ row.huodongzhuangtai }}</template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            ...
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/lvyouhuodongdetail',query:{id:row.id } })"
                                           icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit"
                                           @click="$goto({path:'/admin/lvyouhuodongupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.lvyouhuodong.list, filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItem(row) {
                this.$confirm("确定删除数据？", "提示", {
                    ...
                    .then(() => {
                        ...
                        this.$post(api.lvyouhuodong.delete, {
                            // 提交后台
                            id: row.id,
                        })
                            .then((res) => {
