        ...
        <el-card class="box-card">
                ...
                <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-form-item label="分类">
                        ...
                        >
                            ...
                            >
                            <e-select-option type="option" module="shangpinfenlei" value="id"
                                             label="fenleimingcheng"></e-select-option
                            >
                        </el-select>
                    </el-form-item>
                    <el-form-item>
            ...
            <el-table border :data="list" style="width: 100%" highlight-current-row>
                ...
                <el-table-column label="商品图片" width="100">
                    <template slot-scope="{row}">
                        <e-img :src="row.shangpintupian" type="list" style="max-width: 120px"></e-img>
                    </template>
                </el-table-column>
                <el-table-column label="商品视频">
                    <template slot-scope="{row}">
                        <e-video-list :src="row.shangpinshipin"></e-video-list>
                    </template>
                </el-table-column>
                <el-table-column label="商品价格" width="80">
                    <template slot-scope="{row}"> {{ row.shangpinjiage }}</template>
                </el-table-column>
                <el-table-column label="商品销量" width="80">
                    <template slot-scope="{row}"> {{ row.shangpinxiaoliang }}</template>
                </el-table-column>
                <el-table-column label="商品型号">
                    <template slot-scope="{row}"> {{ row.shangpinxinghao }}</template>
                </el-table-column>
                <el-table-column label="商品颜色">
                    <template slot-scope="{row}"> {{ row.shangpinyanse }}</template>
                </el-table-column>
                <el-table-column label="点击量" width="80">
                    <template slot-scope="{row}"> {{ row.dianjiliang }}</template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.shangpinxinxi.list, filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItem(row) {
                this.$confirm("确定删除数据？", "提示", {
                    ...
                    .then(() => {
                        ...
                        this.$post(api.shangpinxinxi.delete, {
                            // 提交后台
                            id: row.id,
                        })
                            .then((res) => {
